...
contract TopDog is Ownable {
    ...
    function pendingBone(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 boneReward =
                multiplier.mul(bonePerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accBonePerShare = accBonePerShare.add(
                boneReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accBonePerShare = pool.accBonePerShare.add(
            boneReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accBonePerShare).div(1e12).sub(
                    user.rewardDebt
                );
