...
contract TopDogV2 is BoringOwnable, BoringBatchable {
    ...
    uint256 private constant MASTERCHEF_BONE_PER_BLOCK = 1e20;
    ...
    constructor(ITopDog _MASTER_CHEF, IERC20 _bone, uint256 _MASTER_PID) public {
        MASTER_CHEF = _MASTER_CHEF;
        BONE = _bone;
        MASTER_PID = _MASTER_PID;
    }

    /// @notice Deposits a dummy token to `MASTER_CHEF` TDV1. This is required because TDV1 holds the minting rights for BONE.
    ...
    function bonePerBlock() public view returns (uint256 amount) {
        amount = uint256(MASTERCHEF_BONE_PER_BLOCK)
    ...
    /// @notice Harvests BONE from `MASTER_CHEF` TDV1 and pool `MASTER_PID` to this TDV2 contract.
