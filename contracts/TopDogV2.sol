...
contract TopDogV2 is BoringOwnable, BoringBatchable {
    ...
    ITopDog public immutable MASTER_CHEF;
    ...
    /// @param _MASTER_CHEF The ShibaSwap TDV1 contract address.
    ...
    function setMigrator(IMigratorShibaSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function pendingBone(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBonePerShare = accBonePerShare.add(boneReward.mul(ACC_SUSHI_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accBonePerShare) / ACC_SUSHI_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                uint256 blocks = block.number.sub(pool.lastRewardBlock);
                uint256 boneReward = blocks.mul(bonePerBlock()).mul(pool.allocPoint) / totalAllocPoint;
                pool.accBonePerShare = pool.accBonePerShare.add((boneReward.mul(ACC_SUSHI_PRECISION) / lpSupply).to128());
            }
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        ...
        user.rewardDebt = user.rewardDebt.add(int256(amount.mul(pool.accBonePerShare) / ACC_SUSHI_PRECISION));
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = user.rewardDebt.sub(int256(amount.mul(pool.accBonePerShare) / ACC_SUSHI_PRECISION));
    ...
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedBone = int256(user.amount.mul(pool.accBonePerShare) / ACC_SUSHI_PRECISION);
        ...
        // Interactions
        if (_pendingBone != 0) {
            SUSHI.safeTransfer(to, _pendingBone);
        }
    ...
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = accumulatedBone.sub(int256(amount.mul(pool.accBonePerShare) / ACC_SUSHI_PRECISION));
        user.amount = user.amount.sub(amount);
