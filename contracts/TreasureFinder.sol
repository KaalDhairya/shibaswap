...
// TreasureFinder is TopDogs's left hand and kinda a wizard. He can cook up Bone from pretty much anything!
// This contract handles "serving up" rewards for xBone holders by trading tokens collected from fees for Bone.

// T1 - T4: OK
contract TreasureFinder is Ownable {
    ...
    constructor (address _factory, address _bar, address _bone, address _weth) public {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(token != bone && token != weth && token != bridge, "TreasureFinder: Invalid bridge");
    ...
    function _convertStep(address token0, address token1, uint256 amount0, uint256 amount1) internal returns (uint256 boneOut) {
        // Interactions
        if (token0 == token1) {
            uint256 amount = amount0.add(amount1);
            if (token0 == bone) {
                IERC20(bone).safeTransfer(bar, amount);
                boneOut = amount;
            } else if (token0 == weth) {
            ...
            // eg. BONE - ETH
            IERC20(bone).safeTransfer(bar, amount0);
            boneOut = _toBONE(token1, amount1).add(amount0);
        } else if (token1 == bone) {
            // eg. USDT - BONE
            IERC20(bone).safeTransfer(bar, amount1);
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _toBONE(address token, uint256 amountIn)
        internal
        returns (uint256 amountOut)
    {
        // X1 - X5: OK
        amountOut = _swap(token, bone, amountIn, bar);
    }
}
